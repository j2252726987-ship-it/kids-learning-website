...
import { sanziJingSections, SanziJingSection, SanziJingCharacter } from '@/lib/sanzi-jing-data';
...
export default function SanziJingGamePage() {
  ...
  const initializeLevels = () => {
    const savedProgress = localStorage.getItem('sanziJingGameProgress');
    if (savedProgress) {
      setLevels(JSON.parse(savedProgress));
    } else {
      const initialLevels: Level[] = [
        { id: 1, title: '第一关：人之初', sectionId: 'section-1', gameType: 'match', unlocked: true, stars: 0 },
        { id: 2, title: '第二关：教之道', sectionId: 'section-2', gameType: 'listen', unlocked: false, stars: 0 },
        { id: 3, title: '第三关：孟母三迁', sectionId: 'section-3', gameType: 'fill-blank', unlocked: false, stars: 0 },
        { id: 4, title: '第四关：玉不琢', sectionId: 'section-7', gameType: 'match', unlocked: false, stars: 0 },
        { id: 5, title: '第五关：三才者', sectionId: 'section-13', gameType: 'listen', unlocked: false, stars: 0 },
      ];
      setLevels(initialLevels);
      localStorage.setItem('sanziJingGameProgress', JSON.stringify(initialLevels));
    }
  };

  const saveProgress = (updatedLevels: Level[]) => {
    localStorage.setItem('sanziJingGameProgress', JSON.stringify(updatedLevels));
  ...
  const gameTypes = [
    ...
    { id: 'fill-blank', title: '填空挑战', icon: '✍️', description: '补全三字经句子' },
  ] as const;
  ...
  if (currentView === 'playing' && selectedLevel) {
    const section = sanziJingSections.find(s => s.id === selectedLevel.sectionId)!;
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-12 max-w-2xl mx-4 text-center">
          ...
          <div className="flex justify-center gap-6">
            <button
              ...
              className="px-8 py-4 bg-gradient-to-r from-blue-400 to-purple-400 text-white rounded-full text-xl font-bold shadow-xl hover:shadow-2xl transition-all hover:scale-105"
            >
              选择关卡
            </button>
            <button
...
function Game({
  ...
  const generateMatchOptions = (target: SanziJingCharacter, allChars: SanziJingCharacter[]) => {
  ...
  const generateListenOptions = (target: SanziJingCharacter, allChars: SanziJingCharacter[]) => {
    const wrongOptions = allChars
      .filter(c => c.char !== target.char)
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);
  ...
  const generateFillBlankQuestions = (section: SanziJingSection) => {
    const sentences = section.content.split('。').filter((s: string) => s.length > 0);
    return sentences.slice(0, 4).map((sentence: string) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100">
      <header className="bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 py-6 text-center shadow-lg">
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl p-8">
      ...
      <div className="text-center mb-8">
        <div className="inline-block bg-gradient-to-br from-blue-100 to-purple-100 rounded-3xl p-12 border-4 border-blue-300">
    ...
    <div className="bg-white rounded-3xl shadow-xl p-8">
      ...
      <div className="text-center mb-8">
        <button
          ...
          className="inline-flex items-center gap-3 px-8 py-6 bg-gradient-to-r from-blue-400 to-purple-400 text-white rounded-full text-2xl font-bold shadow-xl hover:shadow-2xl transition-all hover:scale-105"
        >
...
function FillBlankGame({ question, isAnswered, showFeedback, isCorrect, onAnswer }: any) {
  return (
    <div className="bg-white rounded-3xl shadow-xl p-8">
      ...
      <div className="text-center mb-8">
        <div className="bg-gradient-to-br from-blue-100 to-purple-100 rounded-3xl p-8 border-4 border-blue-300">
          <div className="text-4xl font-bold text-gray-800 leading-relaxed">
            {question.sentence.split('').map((char: string, index: number) => (
              <span key={index} className={index === question.blankIndex ? 'inline-block min-w-[1.5em] border-b-4 border-blue-400 text-blue-500' : ''}>
