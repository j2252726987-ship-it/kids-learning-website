...
export default function TangshiSpellingLevelSelectorPage() {
  ...
  const loadLevels = () => {
    ...
    const STORAGE_KEY = 'tangshi_spelling_progress';
    ...
    for (let i = 1; i <= totalLevelCount; i++) {
      const level = generateDynamicLevel('spelling', i, 'tangshi');
      if (level) {
        // 从保存的数据中恢复进度
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-rose-50 to-pink-50 p-4">
      <div className="container mx-auto max-w-4xl">
        ...
        {/* 游戏标题和进度 */}
        <div className="mb-8 rounded-3xl bg-gradient-to-r from-green-400 to-emerald-500 p-8 shadow-2xl text-white">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="mb-2 text-4xl font-bold">唐诗拼写挑战 - 关卡选择</h1>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {levels.map((level, index) => (
            <Link
              key={level.id}
              href={`/games/spelling/play?level=${level.id}&tangshi=true`}
        ...
        {/* 无限关卡提示 */}
        {levels.length > 0 && (
          <div className="mt-8 rounded-3xl bg-gradient-to-r from-green-100 to-emerald-100 p-8 shadow-xl border-2 border-green-200">
            <div className="flex items-start gap-3">
              <span className="text-3xl">∞</span>
              <div>
                <h3 className="font-bold text-lg text-green-800">无限关卡挑战</h3>
                <p className="text-green-700">
                  唐诗拼写挑战游戏支持无限动态生成关卡！每关都会从唐诗三百首中精选不同的诗句，
