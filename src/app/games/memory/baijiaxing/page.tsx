...
export default function BaijiaxingMemoryLevelSelectorPage() {
  ...
  const loadLevels = () => {
    const allLevels = getLevelsWithAutoGenerate('memory');
    // 过滤出百家姓相关的关卡
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="mb-8 rounded-3xl bg-gradient-to-r from-green-500 to-emerald-500 p-8 shadow-2xl text-white">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="mb-2 text-4xl font-bold">百家姓记忆翻牌 - 关卡选择</h1>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {levels.map((level, index) => (
            <Link
              key={level.id}
              href={`/games/memory/play?level=${level.id}`}
