...
export default function TangshiQuizLevelSelectorPage() {
  ...
  const loadLevels = () => {
    ...
    const STORAGE_KEY = 'tangshi_quiz_progress';
    ...
    for (let i = 1; i <= totalLevelCount; i++) {
      const level = generateDynamicLevel('quiz', i, 'tangshi');
      if (level) {
        // 从保存的数据中恢复进度
  ...
  const saveProgress = (levels: Level[]) => {
    ...
    localStorage.setItem('tangshi_quiz_progress', JSON.stringify(progress));
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-rose-50 to-pink-50 p-4">
      <div className="container mx-auto max-w-4xl">
        ...
        {/* 游戏标题和进度 */}
        <div className="mb-8 rounded-3xl bg-gradient-to-r from-red-400 via-rose-400 to-pink-400 p-8 shadow-2xl text-white">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="mb-2 text-4xl font-bold">唐诗发音测试 - 关卡选择</h1>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {levels.map((level, index) => (
            <Link
              key={level.id}
              href={`/games/quiz/play?level=${level.id}&tangshi=true`}
        ...
        {/* 无限关卡提示 */}
        {levels.length > 0 && (
          <div className="mt-8 rounded-3xl bg-gradient-to-r from-rose-100 to-pink-100 p-8 shadow-xl border-2 border-rose-200">
            <div className="flex items-start gap-3">
              <span className="text-3xl">∞</span>
              <div>
                <h3 className="font-bold text-lg text-rose-800">无限关卡挑战</h3>
                <p className="text-rose-700">
                  唐诗闯关游戏支持无限动态生成关卡！每关都会从唐诗三百首中精选不同的诗句，
